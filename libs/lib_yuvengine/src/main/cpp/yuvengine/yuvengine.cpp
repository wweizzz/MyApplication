 ...
 * cutW,cutH 裁剪的分辨率
**/
void YuvEngine::cutCommonYuv(int yuvType, int startX, int startY, unsigned char *srcYuv, int srcW,
                             int srcH, unsigned char *tarYuv, int cutW, int cutH) {
    ...
    switch (yuvType) {
        ...
        case FORMAT_NV12: {
            ...
            for (i = 0; i < cutH / 2; i++) {
                //逐行拷贝UV分量
                memcpy(cutUV + i * cutW, srcYuv + startX + srcW * srcH + (i + startY / 2) * srcW,
                       cutW);
            }
        ...
        case FORMAT_I420: {
            ...
            for (i = 0; i < cutH / 2; i++) { //每次循环一次，扫描一行数据
                //拷贝U分量
                memcpy(destU + i * (cutW / 2), srcU + startX / 2 + (i + startY / 2) * (srcW / 2),
                       cutW / 2);
                //拷贝V分量
                memcpy(destV + i * (cutW / 2), srcV + startX / 2 + (i + startY / 2) * (srcW / 2),
                       cutW / 2);
            }
#ifdef DUMP_OUTPUT
            char output[256];
...
**/
void
YuvEngine::getSpecYuvBuffer(int yuvType, unsigned char *dstBuf, unsigned char *srcYuv, int srcW,
                            int srcH, int dirty_Y, int dirty_UV) {
    int i;
    switch (yuvType) {
        case FORMAT_NV21:
        case FORMAT_NV12: {
...
void YuvEngine::yuvAddWaterMark(int yuvType, int startX, int startY, unsigned char *waterMarkData,
                                int waterMarkW, int waterMarkH, unsigned char *yuvData, int yuvW,
                                int yuvH) {
    ...
    if (yuvW < startX + waterMarkW || (yuvH < startY + waterMarkH)) {
        LOGE("%s: startX: %d, startY: %d waterMarkW: %d waterMarkH: %d yuvW: %d yuvH: %d",
             __FUNCTION__, startX, startY, waterMarkW, waterMarkH, yuvW, yuvH);
        return;
    }
    ...
    switch (yuvType) {
        ...
        case FORMAT_NV12: {
            ...
            for (i = startY / 2; i < (waterMarkH + startY) / 2; i++) {
                ...
                memcpy(yuvData + startX + yuvW * yuvH + i * yuvW,
                       waterMarkData + waterMarkW * waterMarkH + k * waterMarkW, waterMarkW);
                k++;
            }
        ...
        case FORMAT_I420: {
            ...
            for (i = 0; i < waterMarkH; i++) { //每次循环一次，扫描一行数据
                memcpy(destY + startX + (i + startY) * yuvW, waterY + i * waterMarkW,
                       waterMarkW); //y值覆盖
            }

            for (i = 0; i < waterMarkH / 2; i++) { //每次循环一次，扫描一行数据
                //拷贝U分量
                memcpy(destU + (i + startY / 2) * (yuvW / 2) + startX / 2,
                       waterU + i * (waterMarkW / 2), waterMarkW / 2);
                //拷贝V分量
                memcpy(destV + (i + startY / 2) * (yuvW / 2) + startX / 2,
                       waterV + i * (waterMarkW / 2), waterMarkW / 2);
            }
...
void YuvEngine::Nv21ClockWiseRotate90(unsigned char *pNv21, int srcWidth, int srcHeight,
...
void YuvEngine::Nv21ClockWiseRotate180(unsigned char *pNv21, int srcWidth, int srcHeight,
                                       unsigned char *outData, int *outWidth, int *outHeight) {
    if (pNv21 == NULL || outData == NULL) {
        LOGE("%s: pNv21 is null or outData is null", __FUNCTION__);
        return;
...
void YuvEngine::Nv21ClockWiseRotate270(unsigned char *pNv21, int srcWidth, int srcHeight,
                                       unsigned char *outData, int *outWidth, int *outHeight) {
    // Rotate the Y luma
    int i = 0;
...
void YuvEngine::I420ClockWiseRotate90(unsigned char *pI420, int srcWidth, int srcHeight,
                                      unsigned char *outData, int *outWidth, int *outHeight) {
...
void YuvEngine::Nv12ClockWiseRotate90(unsigned char *pNv12, int srcWidth, int srcHeight,
                                      unsigned char *outData, int *outWidth, int *outHeight) {
...
void YuvEngine::Yv12ClockWiseRotate90(unsigned char *pYv12, int srcWidth, int srcHeight,
                                      unsigned char *outData, int *outWidth, int *outHeight) {
